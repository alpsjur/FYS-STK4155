...
\end{equation}
Here, $\boldsymbol{I}$ is the identity matrix.. The hyperparameter $\lambda$ then has to be tuned, for example by the use of cross-validation.
...
\end{equation}  
Here, $v_i$ is the $i$th diagonal element of $\left(\boldsymbol{X}^T\boldsymbol{X}\right)^{-1}$ and $z^{1-\alpha}$ is the $1-\alpha$ percentile of the normal distribution. For a 95\% confidence interval, $z^{1-\alpha}$ equals 1.96.
...
\begin{algorithm}[htbp]\caption{The bootstrap algorithm.}\label{alg:bootstrap}
	...
	\ForEach{$n$}{
		...
		Apply the model on the test set, and store the prediction\;
	}
...
The full code can be found in a github repository \footnote{\url{https://github.com/janadr/FYS-STK4155/tree/master/project1}}.
